...
useEffect(async ()=> {
  ...
  await dispatch(getMyEvents(idUser,searchValue))
  ...
  <div className="search-area ptb-50">
        <div className="container">
          <div className="search-block">
            <form className="search-box" style= {{marginTop: "100px", marginLeft:"200px"}} >
              <input
                type="text"
...
{res.myEvents.data && res.myEvents.data.map((item, index)=>{
  return (
    ...
    <Card sx={{ maxWidth: 345 }}>
      ...
      >
        <Button sx={{ p: 2 }}   href={`/EditEvent/${item._id}`} >Edit</Button><br/>
 

        <Button sx={{ p: 2 }} onClick={()=>Delete(item._id)}>Delete</Button>
